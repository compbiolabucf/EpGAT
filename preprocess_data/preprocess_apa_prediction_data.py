...
parser.add_argument('--pred_file_name', type=str, default='APA_Scan_MCF-7_S3.xlsx')
...
save_dir = f'{args.save_dir}/{args.cell_line}/apa_pred_data/'
...
for cnum in range(1,23):
    ...
    print(f'\nProcessing APA for {chr_nm}...')

    out_data.sort_values(by='Start', inplace=True)
    out_chr = out_data.loc[out_data['Chrom']==chr_nm]
    out_chr['Length'] = out_chr['End']-out_chr['Start']

    bin_start = np.arange(1,chr_len, 200)
